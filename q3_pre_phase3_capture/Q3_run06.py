...
cert_gap_emp_ids = set(
    df_training_compliance[
        (df_training_compliance['delivery_mode'] == 'External Certification') &
        (df_training_compliance['status'] != 'Completed')
    ]['employee_id'].dropna()
)

result = (
    no_esop[no_esop['employee_id'].isin(cert_gap_emp_ids)][
